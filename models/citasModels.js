...
const getAllCitas = async () => {
  const result = await pool.query(`SELECT c.id as cita_id,cl.id as cliente_id,cl.nombre as cliente_nombre, cl.apellido as cliente_apellido, cl.telefono, c.fecha, e.nombre as encargado_nombre, e.apellido as encargado_apellido, p.tipo_procedimiento, c.notas, c.mapping_estilo, c.tamaño, c.curvatura, c.espessura
  FROM citas as c
...
const getCitasByTel = async (tel) => {
  ...
  const result = await pool.query(`SELECT citas.id as citas_id, clientes.id as cliente_id cliente.nombre,cliente.telefono,citas.fecha,citas.encargado, citas.procedimiento, citas.notas, citas.mapping_estilo, citas.tamaño, citas.curvatura, citas.espessura
  FROM  citas
  JOIN clientes ON citas.cliente = clientes.id
  WHERE clientes.telefono LIKE $1
  ORDER BY citas.fecha DESC;`, [phone])
  return result;
};
...
const createCitas = async (citas) => {
  ...
  const conflictCheck = await pool.query(
    "SELECT * FROM appointments WHERE fecha = $1 AND (encargado = $2 OR cliente = $3)",
    [
      fecha,
      encargado,
      cliente,
